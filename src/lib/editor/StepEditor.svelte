<script lang="ts">
  ...
  const set = (field: string, value: unknown) =>
    store.apply((d) =>
      setField(d, { blockId: block.block_id, stepId: step.id, field }, value),
    );
  ...
  function confirmQuestionTypeChange() {
    ...
    store.apply((d) =>
      setQuestionType(d, block.block_id, step.id, pendingTypeChange!.type),
    );
...
>
  <header>
    ...
    <Button
      ...
      size="s"
      onclick={() =>
        store.apply((d, r) => duplicateStep(d, block.block_id, step.id, r))}
    >
      <Copy size={16}></Copy>
      Duplikovat
    </Button>
  ...
  {#if !collapsed}
      ...
      <div class="media">
        ...
        {#if videoIsPageLink}
          ...
          <p class="field-hint warning">
            Tohle je odkaz na stránku YouTube/Vimeo, ne na video samotné —
            přehrávač v kurzu ho nenačte. Otevři video, najdi jeho přímý soubor
            (.mp4) a vlož adresu toho.
          </p>
        {/if}
        ...
        {#if step.question?.type === "open"}
          <div class="field-row">
            ...
            <FocusField
              ...
              invalid={issues.errors.some(
                (i) => i.code === "E_OPEN_NO_CORRECT_ANSWER",
              )}
        ...
        {:else if step.question?.type === "numeric"}
          <div class="field-row">
            ...
            <FocusField
              ...
              monospace
              invalid={issues.errors.some(
                (i) => i.code === "E_NUMERIC_NO_CORRECT_NUMBER",
              )}
              onchange={(v) =>
                set(
    ...
    {#each [...issues.errors, ...issues.warnings] as issue (issue.code + refKey(issue.ref))}
      {#if issue.ref.optionId === undefined}
        <p class="issue" class:warning={issue.severity === "warning"}>
          {issue.message}
        </p>
      {/if}
    {/each}
      ...
      <Modal title="Změnit typ otázky?" onclose={cancelQuestionTypeChange}>
        ...
        {#snippet footer()}
          <Button variant="ghost" onclick={cancelQuestionTypeChange}
            >Zrušit</Button
          >
          <Button variant="danger-solid" onclick={confirmQuestionTypeChange}
