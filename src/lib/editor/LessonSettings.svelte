<script lang="ts">
	...
	const didactics = $derived(lesson === undefined ? undefined : lessonDidactics(lesson, store.index));
		...
		{#if totals !== undefined && didactics !== undefined}
			<section>
				...
				{#if didactics.wrongOptionFeedbackShare < 0.5}
					<p class="warn">
						Většina chybných odpovědí žákovi neřekne, kde udělal chybu. Zpětná vazba u
						možnosti je to jediné, co z chyby udělá učení.
					</p>
				{/if}
			</section>
		{/if}
