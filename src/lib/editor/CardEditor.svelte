<script lang="ts">
  ...
  const activeNotice = $derived(
    notice !== null && store.undoStack.at(-1) === notice.entry ? notice : null,
  );
  ...
  function removeFromLesson() {
    ...
    const name =
      doc.lessons.find((lesson) => lesson.lesson_id === fromLessonId)?.name ??
      fromLessonId;
  ...
  function assignToLesson() {
    if (!availableLessons.some((lesson) => lesson.lesson_id === targetLessonId))
      return;
    const blockId = block.block_id;
  ...
  <header>
    ...
    {#if xpIsDerived}
      <!--
			...
			-->
      <span class="xp-note">
        automaticky z kroků: 8 XP za každý krok s otázkou, 1 XP za obsahový krok
        — platí hned, i když je karta ještě rozepsaná. Vlastní hodnotu nastavíš
        v Nastavení karty.
      </span>
    {/if}
    ...
    <Button
      ...
      size="s"
      onclick={() =>
        store.apply((d, r) => duplicateBlock(d, block.block_id, lessonId, r))}
    >
      <Copy size={16}></Copy>
      Duplikovat
    </Button>
