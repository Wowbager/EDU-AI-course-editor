...
<header class="topbar">
	...
	<button type="button" class="chip-button" onclick={onvalidation}>
		{#if store.validation.errors.length > 0}
			<Chip tone="error">{errorsCount(store.validation.errors.length)}</Chip>
		{:else if store.validation.warnings.length > 0}
			<Chip tone="warning">{warningsCount(store.validation.warnings.length)}</Chip>
		{:else}
...
{#if explainStorage}
	<Modal title="Kde je kurz uložený" onclose={() => (explainStorage = false)}>
		<div class="storage">
			<p>
				Rozepsaný kurz se průběžně ukládá <strong>do tohoto prohlížeče</strong>, ne na
				server. Nikdo jiný k němu nemá přístup a ty se k němu nedostaneš z jiného počítače
				ani z jiného prohlížeče.
			</p>
			<p>
				Vymazání dat stránky, anonymní okno nebo přeinstalace prohlížeče kurz nenávratně
				smaže. Jediná záloha, kterou máš, je stažený soubor JSON — ten si můžeš kdykoli
				zase načíst tlačítkem <strong>Načíst</strong>.
			</p>
			...
			{#if !store.canPublish}
				<p>
					Stáhnout JSON teď nejde — kurz má {errorsCount(store.validation.errors.length)}, které by
					žákovi rozbily lekci. Oprav je a zálohu si stáhni hned poté.
				</p>
			{/if}
		</div>
	</Modal>
