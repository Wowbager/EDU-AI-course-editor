...
<style>
	...
	.field.compact { background: none; text-overflow: ellipsis; }
	.field::placeholder { color: var(--e-text-faint); font-style: italic; }
	.field:placeholder-shown { border-bottom-style: dashed; }
	.field:hover:not(:disabled) { background: var(--e-field-hover); }
	.field:focus {
	...
	.field.invalid { box-shadow: inset 2px 0 0 var(--e-error); }
	.field:disabled { background: none; color: var(--e-text-muted); }
	/*
	 ...
	 */
	.mono { font-family: var(--font-code); font-size: var(--text-s); letter-spacing: 0.03em; }
</style>
