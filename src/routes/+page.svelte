<script lang="ts">
    ...
    const lesson = $derived.by(() => {
        ...
        if (selection?.blockId !== undefined) {
            const owner = (store.index.lessonsByBlock.get(selection.blockId) ??
                [])[0];
            if (owner !== undefined)
                return doc.lessons.find((l) => l.lesson_id === owner);
            // A card in no lesson at all. Belonging to the first lesson would be a lie.
            if (doc.blocks.some((b) => b.block_id === selection.blockId))
                return undefined;
        }
    ...
    const card = $derived(
        selectedBlock ??
            (lesson !== undefined
                ? doc.blocks.find(
                      (b) => b.block_id === lesson.blocks[0]?.block_id,
                  )
                : undefined),
    );
    const binding = $derived(
        card === undefined
            ? undefined
            : lesson?.blocks.find((b) => b.block_id === card.block_id),
    );
    ...
    async function onimport(file: File) {
        ...
        try {
            ...
            if (looksLikeGpfTaxonomy(parsed)) {
                store.skillConfig = skillConfigFromGpfTaxonomy(
                    parsed,
    ...
    async function loadConfig() {
        ...
        if (config !== null && store.doc.course_id === courseId)
    ...
    $effect(() => {
        ...
        if (
            selected !== undefined &&
            current.blocks.some((b) => b.block_id === selected)
        )
            return;
    ...
    {#if recovery?.message}
        <div class="recovery" role="status">
            ...
            {#if recovery.status === "blocked"}
                <button
                    ...
                    onclick={() => {
                        if (
                            confirm(
                                "Zálohovat původní koncept a ukládat místo něj tento kurz?",
                            )
                        )
                            recovery?.replace();
            ...
            {:else if recovery.status === "error"}
                <button type="button" onclick={() => recovery?.flush()}
                    >Zkusit uložit znovu</button
                >
            {/if}
            ...
            <button
                type="button"
                class="close"
                onclick={() => (recovery = null)}><X size={16}></X></button
            >
        </div>
    {/if}
    ...
    <div class="columns">
        ...
        <main class="editor">
            ...
            {#if importNotes.length > 0}
                <div class="banner">
                    <strong
                        >Při načtení se něco převedlo ({importNotes.length}):</strong
                    >
                    <ul>
